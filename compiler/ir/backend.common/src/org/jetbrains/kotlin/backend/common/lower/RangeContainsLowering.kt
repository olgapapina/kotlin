...
private class Transformer(
    ...
    override fun visitCall(expression: IrCall): IrExpression {
        // The call to contains() in `5 in 0..10` looks like this in IR:
        //
        ...
        //
        // And when `!in` is used in `5 !in 0..10`:
        //
        ...
        //
        // We only want to lower the call to contains(); even in the `!in` case, the call to not() should be preserved).
        val origin = expression.origin
        if (origin == null || (origin != IrStatementOrigin.IN && origin != IrStatementOrigin.NOT_IN)) {
    ...
    private fun DeclarationIrBuilder.buildContainsComparison(
        ...
        var lower: IrExpression
        var upper: IrExpression
